  ...
  try {
    ...
    if (consultation_type === "follow_up" && previous_treatment_plan_id) {
      const [prevPlan] = await db
        .select()
        .from(treatmentPlans)
        .where(eq(treatmentPlans.id, previous_treatment_plan_id));
      if (!prevPlan) {
        return res
          .status(404)
          .json({
            success: false,
            message: "Previous treatment plan not found",
          });
      }
      if (prevPlan.patient_id !== patient_id) {
        return res
          ...
          .json({
            success: false,
            message: "Treatment plan belongs to a different patient",
          });
      }
    }
...
export const getConsultationById = async (req, res) => {
  try {
    ...
    const [patient] = await db
      ...
      .from(users)
      .where(eq(users.id, consultation.patient_id));
    const [appointment] = await db
      .select()
      .from(appointments)
      .where(eq(appointments.id, consultation.appointment_id));

    let previousPlan = null;
    if (
      consultation.consultation_type === "follow_up" &&
      consultation.previous_treatment_plan_id
    ) {
      [previousPlan] = await db
        .select()
        .from(treatmentPlans)
        .where(eq(treatmentPlans.id, consultation.previous_treatment_plan_id));
    }
