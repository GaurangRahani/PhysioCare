...
import { exercises, treatmentPlanExercises, treatmentPlans } from '../src/db/schema/index.js';
import { eq, and } from 'drizzle-orm';

// ─── 1. POST /api/exercises ──────────────────────────────────────────────────
export const createExercise = async (req, res) => {
    console.log('[DEBUG] createExercise called! Files uploaded successfully to Cloudinary.');
    ...
    try {
        ...
        if (req.files && Array.isArray(req.files)) {
            ...
            const photoFiles = req.files.filter(f => f.fieldname === 'photos' || f.fieldname === 'photo');
            if (photoFiles.length > 0) {
                const uploadedPhotoUrls = photoFiles.map(file => file.path);
                photo_urls = Array.isArray(photo_urls) ? [...photo_urls, ...uploadedPhotoUrls] : uploadedPhotoUrls;
            }
        }
...
export const getExercises = async (req, res) => {
    try {
        ...
        if (queryConditions.length > 0) {
            result = await db.select().from(exercises).where(and(...queryConditions));
        } else {
