...
export const getAvailabilityRules = async (req, res) => {
    try {
        ...
        if (!profile) {
            return res.status(404).json({ success: false, message: 'Doctor profile not found. Please update your profile first.' });
        }

        return res.status(200).json({ success: true, availability_rules: profile.availability_rules });
    } catch (error) {
...
export const deleteSpecificDate = async (req, res) => {
    try {
        ...
        if (!updatedRules.specific_dates[date]) {
            return res.status(404).json({ success: false, message: `No override found for ${date}` });
        }
...
export const getAvailableSlots = async (req, res) => {
    try {
        ...
        const today = new Date(new Date().toISOString().split('T')[0] + 'T00:00:00Z'); // Server today in UTC
        ...
        const { slot_minutes, weekly_routine, specific_dates } = profile.availability_rules;
        ...
        if (!workingShifts || workingShifts.length === 0) {
            return res.status(200).json({ success: true, date, available: false, slots: [] });
        }
