...
import { users, patientProfiles, doctorProfiles } from '../src/db/schema/index.js';
...
export const requireAuth = async (req, res, next) => {
  try {
    ...
    let [dbUser] = await db.select().from(users).where(eq(users.clerk_id, auth.userId));
    ...
    if (!dbUser) {
      try {
        ...
        if (clerkUser) {
          ...
          }).onConflictDoNothing().returning(); // In case of race conditions
    ...
    if (!dbUser) {
      return res.status(404).json({ success: false, message: 'User not found in database and JIT sync failed.' });
    }
    ...
    return res.status(500).json({ success: false, message: 'Server error checking authentication.' });
  }
};
